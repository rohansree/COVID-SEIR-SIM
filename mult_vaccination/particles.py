...
class Particles():
    def __init__(self, simulator):
        ...
        self.vac_type = np.empty((simulator.NUMBER_OF_PARTICLES,), dtype='<U10')  #for vaccine type (A, B, C)
        self.vac_dose = np.zeros((simulator.NUMBER_OF_PARTICLES,))  # Integer array for vaccine dose. 
        #0, 1, 2, until more doses are available
        self.vac_effectiveness = np.zeros((simulator.NUMBER_OF_PARTICLES,))  #float ARRAY for vaccine effectiveness
    ...
    def update_states(self, i, simulator):
        ...
        #encoder would be more efficienet
        #encoder implementation was also a work in progress, like the simulator performance upgrades
    ...
    def plot(self, simulator,i):
        ...
        # self.df.plot(x ='days', y='vaccinations', kind = 'line', color = 'pink', ax=ax)
        #plotting vaccinations did not make sense in this context. vaccine count for a particle is not the same as the number of vaccinations given
    ...
    def vac_per_iter(self, i, simulator):
        ...
        #With economics background, this function could be an entire model unto itself
        #vaccine availability per iteration is an enormous economic and logistical challenge in the real world
        #and this simple approximation is not an accurate representation of that
        
        rand_num = np.random.uniform(0, 1, 1)
        if rand_num[0]>(simulator.VAC_FLOAT - simulator.VAC_FLOOR): #too high
            vac_per_iter = simulator.VAC_FLOOR
            # return vac_per_iter
        else:
            vac_per_iter = simulator.VAC_CEIL

        # self.total_vac += vac_per_iter    
        return vac_per_iter
    
    def apply_vaccination(self, i, simulator, vac_iter):
        self.total_vac += vac_iter
